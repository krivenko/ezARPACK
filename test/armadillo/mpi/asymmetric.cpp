...
TEST_CASE("Asymmetric eigenproblem is solved", "[solver_asymmetric]") {
  ...
  using solver_t = mpi::arpack_solver<ezarpack::Asymmetric, armadillo_storage>;
  ...
  SECTION("Generalized eigenproblem: invert mode") {
    ...
    solver_t ar(A.n_rows, MPI_COMM_WORLD);
    const int ncv = 30;
    testing.generalized_eigenproblems(ar, solver_t::Inverse, op, Bop, 0, ncv);
  }

  SECTION("Generalized eigenproblem: Shift-and-Invert mode (real part)") {
    decltype(A) op_mat = real(inv(A - sigma * M) * M);
  ...
  SECTION("Generalized eigenproblem: Shift-and-Invert mode (imaginary part)") {
    decltype(A) op_mat = imag(inv(A - sigma * M) * M);
  ...
  SECTION("Custom implementation of the Exact Shift Strategy") {
    ...
    auto size_f = [](rvv_t shifts) -> int { return shifts.n_rows; };
