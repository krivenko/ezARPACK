...
template<bool Complex> class mpi_mat_vec : public mpi_dist_op_base {
  ...
  mutable arma::Col<scalar_t> M_v_block;
...
public:
  ...
  void operator()(MT const& M, VT const& v, REST& res) const {
    M_v_block =
        M.cols(local_block_start, local_block_start + local_block_size - 1) * v;

    MPI_Datatype datatype = Complex ? MPI_CXX_DOUBLE_COMPLEX : MPI_DOUBLE;

    MPI_Reduce_scatter(M_v_block.memptr(), &res[0], block_sizes.data(),
...
public:
  ...
  scalar_t operator()(VT1 const& v1, VT2 const& v2) const {
    scalar_t partial_dot_prod = cdot(v1, v2);
...
void check_eigenvectors(mpi::arpack_solver<MKind, armadillo_storage> const& ar,
                        M const& A) {
  ...
  arma::Col<typename decltype(vecs)::elem_type> lhs(ar.local_block_size());
...
void check_eigenvectors(mpi::arpack_solver<MKind, armadillo_storage> const& ar,
  ...
  arma::Col<typename decltype(vecs)::elem_type> lhs(ar.local_block_size());
  arma::Col<typename decltype(vecs)::elem_type> rhs(ar.local_block_size());
  for(int i = 0; i < int(lambda.n_elem); ++i) {
    auto vec = vecs.col(i);
    prod(A, vec, lhs);
    prod(M, vec, rhs);
...
void check_eigenvectors_shift_and_invert(
    mpi::arpack_solver<ezarpack::Asymmetric, armadillo_storage> const& ar,
    MT const& A,
    MT const& M) {
  using solver_t = arpack_solver<ezarpack::Asymmetric, armadillo_storage>;
  ...
  arma::Col<dcomplex> lhs(ar.local_block_size());
  arma::Col<dcomplex> rhs(ar.local_block_size());
  for(int i = 0; i < int(lambda.n_elem); ++i) {
    auto vec = vecs.col(i);
...
auto get_basis_vectors(
    mpi::arpack_solver<ezarpack::Symmetric, armadillo_storage> const& ar)
...
auto get_basis_vectors(mpi::arpack_solver<MKind, armadillo_storage> const& ar)
...
void check_basis_vectors(
    mpi::arpack_solver<MKind, armadillo_storage> const& ar) {
...
template<operator_kind MKind, typename MT>
void check_basis_vectors(mpi::arpack_solver<MKind, armadillo_storage> const& ar,
                         MT const& B) {
  auto vecs = get_basis_vectors(ar);
  ...
  arma::Col<typename decltype(vecs)::elem_type> Bvj(ar.local_block_size());
  for(int i = 0; i < int(vecs.n_cols); ++i) {
    auto vi = vecs.col(i);
    for(int j = 0; j < int(vecs.n_cols); ++j) {
      auto vj = vecs.col(j);
