...
public:
  ...
  void operator()(MT const& M, VT const& v, REST& res) const {
    ...
    MPI_Reduce_scatter(&M_v_block(0), &res(0), block_sizes.data(), datatype,
                       MPI_SUM, comm);
  }
};
...
public:
  ...
  scalar_t operator()(VT1 const& v1, VT2 const& v2) const {
    scalar_t partial_dot_prod = inner_prod(conj(v1), v2);
...
void check_eigenvectors(mpi::arpack_solver<MKind, ublas_storage> const& ar,
                        M const& A) {
...
void check_eigenvectors_shift_and_invert(
    mpi::arpack_solver<ezarpack::Asymmetric, ublas_storage> const& ar,
    MT const& A,
    MT const& M) {
...
auto get_basis_vectors(
    mpi::arpack_solver<ezarpack::Symmetric, ublas_storage> const& ar)
...
auto get_basis_vectors(mpi::arpack_solver<MKind, ublas_storage> const& ar)
...
template<operator_kind MKind>
void check_basis_vectors(mpi::arpack_solver<MKind, ublas_storage> const& ar) {
  ...
  for(int i = 0; i < vecs.size2(); ++i) {
    auto vi = column(vecs, i);
    for(int j = 0; j < vecs.size2(); ++j) {
