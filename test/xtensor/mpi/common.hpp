...
public:
  mpi_mat_vec(int N, MPI_Comm const& comm)
      : mpi_dist_op_base(N, comm),
        M_v_block(std::array<size_t, 1>{size_t(N)}) {}

  template<typename MT, typename VT, typename REST>
  void operator()(MT const& M, VT const& v, REST& res) const {
    M_v_block = linalg::dot(
        view(M, all(),
             range(local_block_start, local_block_start + local_block_size)),
        v);

    MPI_Datatype datatype = Complex ? MPI_CXX_DOUBLE_COMPLEX : MPI_DOUBLE;

    MPI_Reduce_scatter(M_v_block.data(), res.data() + res.data_offset(),
                       block_sizes.data(), datatype, MPI_SUM, comm);
  }
};
...
public:
  ...
  scalar_t operator()(VT1 const& v1, VT2 const& v2) const {
    scalar_t partial_dot_prod = linalg::vdot(v1, v2);
...
void check_eigenvectors(mpi::arpack_solver<MKind, xtensor_storage> const& ar,
                        M const& A) {
  ...
  vector<typename decltype(vecs)::value_type> lhs(
      std::array<size_t, 1>{size_t(ar.local_block_size())});
...
void check_eigenvectors(mpi::arpack_solver<MKind, xtensor_storage> const& ar,
  ...
  vector<typename decltype(vecs)::value_type> lhs(
      std::array<size_t, 1>{size_t(ar.local_block_size())});
  vector<typename decltype(vecs)::value_type> rhs(
      std::array<size_t, 1>{size_t(ar.local_block_size())});
  for(int i = 0; i < int(lambda.size()); ++i) {
    auto vec = view(vecs, all(), i);
    prod(A, vec, lhs);
...
void check_eigenvectors_shift_and_invert(
    mpi::arpack_solver<ezarpack::Asymmetric, xtensor_storage> const& ar,
    MT const& A,
    MT const& M) {
  ...
  vector<dcomplex> lhs(std::array<size_t, 1>{size_t(ar.local_block_size())});
  vector<dcomplex> rhs(std::array<size_t, 1>{size_t(ar.local_block_size())});
  for(int i = 0; i < int(lambda.size()); ++i) {
    auto vec = view(vecs, all(), i);
...
auto get_basis_vectors(
    mpi::arpack_solver<ezarpack::Symmetric, xtensor_storage> const& ar)
...
auto get_basis_vectors(mpi::arpack_solver<MKind, xtensor_storage> const& ar)
...
template<operator_kind MKind>
void check_basis_vectors(mpi::arpack_solver<MKind, xtensor_storage> const& ar) {
...
void check_basis_vectors(mpi::arpack_solver<MKind, xtensor_storage> const& ar,
  ...
  vector<typename decltype(vecs)::value_type> Bvj(
      std::array<size_t, 1>{size_t(ar.local_block_size())});
  for(int i = 0; i < int(vecs.shape(1)); ++i) {
    auto vi = view(vecs, all(), i);
    for(int j = 0; j < int(vecs.shape(1)); ++j) {
