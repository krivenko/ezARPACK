...
public:
  ...
  void operator()(MT const& M, VT const& v, REST& res) const {
    M_v_block =
        blaze::submatrix(M, 0, local_block_start, M.rows(), local_block_size) *
        v;
...
public:
  ...
  scalar_t operator()(VT1 const& v1, VT2 const& v2) const {
    scalar_t partial_dot_prod = dot(conj(v1), v2);
...
void check_eigenvectors(mpi::arpack_solver<MKind, blaze_storage> const& ar,
                        M const& A) {
  ...
  vector<typename decltype(vecs)::ElementType> lhs(ar.local_block_size());
...
void check_eigenvectors(mpi::arpack_solver<MKind, blaze_storage> const& ar,
  ...
  vector<typename decltype(vecs)::ElementType> lhs(ar.local_block_size());
  vector<typename decltype(vecs)::ElementType> rhs(ar.local_block_size());
  for(int i = 0; i < int(lambda.size()); ++i) {
    auto vec = column(vecs, i);
    prod(A, vec, lhs);
    prod(M, vec, rhs);
...
void check_eigenvectors_shift_and_invert(
    mpi::arpack_solver<ezarpack::Asymmetric, blaze_storage> const& ar,
    MT const& A,
    MT const& M) {
  using solver_t = arpack_solver<ezarpack::Asymmetric, blaze_storage>;
...
auto get_basis_vectors(
    mpi::arpack_solver<ezarpack::Symmetric, blaze_storage> const& ar)
...
auto get_basis_vectors(mpi::arpack_solver<MKind, blaze_storage> const& ar)
...
template<operator_kind MKind>
void check_basis_vectors(mpi::arpack_solver<MKind, blaze_storage> const& ar) {
...
void check_basis_vectors(mpi::arpack_solver<MKind, blaze_storage> const& ar,
  ...
  vector<typename decltype(vecs)::ElementType> Bvj(ar.local_block_size());
  for(int i = 0; i < int(vecs.columns()); ++i) {
    auto vi = column(vecs, i);
    for(int j = 0; j < int(vecs.columns()); ++j) {
      auto vj = column(vecs, j);
