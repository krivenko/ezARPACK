...
public:
  ...
  void operator()(MT const& M, VT const& v, REST& res) const {
    ...
    MPI_Reduce_scatter(M_v_block.data_start(), res.data_start(),
...
public:
  ...
  scalar_t operator()(VT1 const& v1, VT2 const& v2) const {
    scalar_t partial_dot_prod = dotc(v1, v2);
...
void check_eigenvectors(
    ezarpack::mpi::arpack_solver<MKind, triqs_storage> const& ar,
    M const& A) {
...
void check_eigenvectors_shift_and_invert(
    ezarpack::mpi::arpack_solver<Asymmetric, triqs_storage> const& ar,
    MT const& A,
    MT const& M) {
...
auto get_basis_vectors(
    ezarpack::mpi::arpack_solver<Symmetric, triqs_storage> const& ar)
