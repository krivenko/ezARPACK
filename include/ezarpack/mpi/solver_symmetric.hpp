 ...
 *
 ******************************************************************************/
/// @file ezarpack/mpi/solver_symmetric.hpp
/// @brief Specialization of `mpi::arpack_solver` class for the case of real
...
/// Method (IRLM) for real symmetric eigenproblems (MPI version).
///
/// This specialization of `arpack_solver` calls ARPACK-NG routines `pdsaupd()`
/// and `pdseupd()` to compute approximations to a few eigenpairs of a linear
...
private:
  ...
  int rvec;                   // RVEC parameter of pdseupd
  int_vector_t select;        // SELECT parameter of pdseupd
...
public:
  ...
  /// reported by ARPACK-NG routines `pdsaupd()` and `pdseupd()`.
  ...
  void operator()(A&& a, params_t const& params, ShiftsF shifts_f = {}) {
    ...
    Bx_available_ = false;
    do {
      f77::paupd<true>(comm, ido, "I", block_size, which, nev, tol,
    ...
    f77::peupd(comm, rvec, "A", storage::get_data_ptr(select),
               storage::get_data_ptr(d), storage::get_data_ptr(v), ldv,
               params.sigma, "I", block_size, which, nev, tol,
  ...
  enum Mode : int {
    ...
    */
    ...
    @f$ \hat O = (\hat K - \sigma\hat K_G)^{-1} \hat K @f$
  ...
  /// those ranks and to appropriately update `in` and `out` on all ranks.
  ...
  void operator()(OP&& op,
    ...
    Bx_available_ = false;
    do {
      f77::paupd<true>(comm, ido, "G", block_size, which, nev, tol,
                       storage::get_data_ptr(resid), ncv,
                       storage::get_data_ptr(v), ldv, iparam, ipntr,
                       storage::get_data_ptr(workd),
                       storage::get_data_ptr(workl), workl_size, info);
      switch(ido) {
        case ApplyOpInit: {
          ...
          int out_pos = out_vector_n() * block_size;
          Bx_available_ = false;
          op(storage::make_vector_view(workd, in_pos, block_size),
             storage::make_vector_view(workd, out_pos, block_size));
        } break;
        case ApplyOp: {
          int in_pos = in_vector_n() * block_size;
    ...
    f77::peupd(comm, rvec, "A", storage::get_data_ptr(select),
               ...
               "G", block_size, which, nev, tol, storage::get_data_ptr(resid),
               ncv, storage::get_data_ptr(v), ldv, iparam, ipntr,
...
private:
  /// @internal Translate pdsaupd's INFO codes into C++ exceptions.
  ///
  /// @param error_code pdsaupd's INFO code.
    ...
    switch(error_code) {
      ...
      case -9999:
        throw ARPACK_SOLVER_ERROR(
            ...
            std::to_string(nconv()));
      default:
        throw ARPACK_SOLVER_ERROR("pdsaupd failed with error code " +
                                  std::to_string(error_code));
    }
  }

  /// @internal Translate pdseupd's INFO codes into C++ exceptions.
  ///
  /// @param error_code pdseupd's INFO code.
  void handle_peupd_error_codes(int error_code) {
    switch(error_code) {
      ...
      case -14:
        throw ARPACK_SOLVER_ERROR(
            "pdsaupd did not find any eigenvalues to sufficient accuracy");
      default:
        throw ARPACK_SOLVER_ERROR("pdseupd failed with error code " +
                                  std::to_string(error_code));
    }
  }
};
