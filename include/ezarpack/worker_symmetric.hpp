...
private:
  ...
  void handle_aupd_error_codes(int info, real_vector_t & workl) {
    ...
    switch(info) {
      ...
      case -13:
        throw ARPACK_WORKER_ERROR(
          "n_eigenvalues = 1 is incompatible with eigenvalues_select = BothEnds"
        );
      case -9999:
        throw ARPACK_WORKER_ERROR(
      ...
      default:
        throw ARPACK_WORKER_ERROR(
          "dsaupd failed with error code " + std::to_string(info)
        );
    }
  }
  void handle_eupd_error_codes(int info) {
    switch(info) {
      ...
      default:
        throw ARPACK_WORKER_ERROR(
          "dseupd failed with error code " + std::to_string(info)
        );
    }
  }
};
