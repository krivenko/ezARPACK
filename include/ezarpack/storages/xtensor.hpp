...
template<> struct storage_traits<xtensor_storage> {
  ...
  template<typename T> using matrix =
    xt::xtensor<T, 2, xt::layout_type::column_major>;
  ...
  template<typename T> using matrix_view =
    decltype(xt::view(std::declval<matrix<T>&>(),
                      xt::range(0, 0),
                      xt::range(0, 0)
                     )
            );
  template<typename T> using matrix_const_view =
    decltype(xt::view(std::declval<matrix<T> const&>(),
                      xt::range(0, 0),
                      xt::range(0, 0)
                     )
            );
  ...
  inline static real_matrix_type make_real_matrix(int rows, int cols) {
    return real_matrix_type(real_matrix_type::shape_type{size_t(rows),
                                                         size_t(cols)}
                           );
  }
  inline static complex_matrix_type make_complex_matrix(int rows, int cols) {
    return complex_matrix_type(complex_matrix_type::shape_type{size_t(rows),
                                                               size_t(cols)}
                              );
  }
  ...
  // Resize
  template<typename T>
  inline static void resize(vector<T> &v, int size) {
  ...
  complex_matrix_type make_asymm_eigenvectors(real_matrix_type const& z,
    ...
    for(int i = 0; i < nev; ++i) {
      if(di(i) == 0) {
        ...
        if(i < nev-1) {
          xt::view(res, xt::all(), i+1) = xt::conj(xt::view(res, xt::all(), i));
          ++i;
        }
      }
    }
