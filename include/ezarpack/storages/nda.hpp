...
/// @ref ezarpack::arpack_solver instructs it to use nda's vector/matrix/view
/// types.
struct nda_storage {};
...
/// operations.
template<> struct storage_traits<nda_storage> {
private:
  ...
  template<typename T> using vector = nda::vector<T>;
  /// nda matrix.
  /// @tparam T Matrix element type.
  template<typename T> using matrix = nda::matrix<T, nda::F_layout>;

  /// Vector view type.
  /// @tparam T Vector element type.
  template<typename T> using vector_view = nda::vector_view<T>;
  /// Constant vector view type.
  /// @tparam T Vector element type.
  template<typename T> using vector_const_view = nda::vector_const_view<T>;
  /// Matrix view type.
  /// @tparam T Matrix element type.
  template<typename T>
...
public:
  ...
  /// This function is, in fact, no-op, because memory owned by a nda vector
  ...
  /// This function is, in fact, no-op, because memory owned by a nda matrix
  /// will automatically be freed by its destructor.
  /// @tparam T Matrix element type.
