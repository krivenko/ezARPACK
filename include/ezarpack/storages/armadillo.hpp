...
public:
  ...
  /// This function is, in fact, no-op, because memory owned by an Armadillo
  /// column will automatically be freed by its destructor.
  /// @tparam T Vector element type.
  template<typename T> inline static void destroy(vector<T>&) {}
  /// Destroys a matrix container.
  /// This function is, in fact, no-op, because memory owned by an Armadillo
  /// matrix will automatically be freed by its destructor.
  /// @tparam T Matrix element type.
