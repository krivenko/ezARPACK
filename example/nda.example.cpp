...
int main() {
  ...
  arpack_solver<Symmetric, nda_storage> solver(N);

  using vector_view_t = arpack_solver<Symmetric, nda_storage>::vector_view_t;
  using vector_const_view_t =
      arpack_solver<Symmetric, nda_storage>::vector_const_view_t;
  ...
  for(int i = 0; i < N_ev; ++i) {    // For each eigenpair ...
    ...
    std::cout << i << ": deviation = " << sum(abs2(rhs - lhs)) / (N * N)
              << std::endl;
  }
