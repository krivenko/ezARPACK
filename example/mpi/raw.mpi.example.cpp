...
// the raw memory storage backend to partially diagonalize a large sparse
...
int main(int argc, char* argv[]) {
  ...
  // * `mpi::arpack_solver<Asymmetric, raw_storage>' for general real matrices;
  // * `mpi::arpack_solver<Complex, raw_storage>' for general complex matrices.
  ...
  double* local_op_in = new double[N];
  ...
  auto matrix_op = [&](solver_t::vector_const_view_t in,
    ...
    std::fill(local_op_in, local_op_in + N, 0); // Clear result
    for(int i = 0; i < N; ++i) {
    ...
    MPI_Reduce_scatter(local_op_in, out, block_sizes.data(), MPI_DOUBLE,
  ...
  double* lhs = new double[block_size];
  double* rhs = new double[block_size];

  for(int i = 0; i < N_ev; ++i) { // For each eigenpair ...
    auto const eigenvec_block = v + block_size * i;
    matrix_op(eigenvec_block, lhs); // calculate the local block of A*v
    std::transform(eigenvec_block,  // and the local block of \lambda*v
                   eigenvec_block + block_size, rhs,
    ...
    deviation = std::sqrt(deviation) / block_size;
